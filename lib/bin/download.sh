...
__asdf_bin() {
  ...
  if kc_asdf_is_ref; then
    url="${ASDF_OVERRIDE_REF_REPO:-https://github.com/jsonnet-bundler/jsonnet-bundler.git}"
    ...
    url="https://github.com/jsonnet-bundler/jsonnet-bundler/releases/download/v{version}/jb-{os}-{arch}"
    url="$(kc_asdf_template "$url" "${vars[@]}")"
